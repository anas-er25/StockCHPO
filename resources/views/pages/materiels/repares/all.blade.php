...
@section('title', 'Liste de matériels à réparer')
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="flex justify-between items-center">
                                <h2 class="text-xl font-semibold">Liste de matériels à réparer</h2>
                                <div class="flex items-center gap-4">
                                    <form action="{{ route('repare.exportSelectedPDF') }}" method="POST" id="export-form">
                                        @csrf
                                        <button type="submit"
                                    ...
                                    <a href="{{ route('repare.reparePDF') }}" title="Télécharger Canevas PDF"
                            ...
                            <div class="relative overflow-x-auto mt-8">
                                <table id="table" class="w-full text-sm text-left rtl:text-right text-gray-500">
                                    ...
                                    <tbody>
                                        @forelse ($reparelist as $repare)
                                            <tr class="bg-white hover:bg-gray-50 transition-colors duration-200">
                                                <td class="px-6 py-4 text-center">
                                                    <input type="checkbox" name="selected_ids[]" value="{{ $repare->id }}"
                                                        class="select-item">
                                                </td>
                                                ...
                                                <td class="px-6 py-4 flex items-center justify-center">
                                                    ...
                                                    <a href="{{ route('repare.repareedit', $repare->id) }}"
                                                        class="cursor-pointer mr-4">
                                                        <i class="fa-solid fa-pen text-blue-600 hover:text-blue-700"></i>
                                                    </a>

                                                    <!-- Formulaire de suppression -->
                                                    <form action="{{ route('repare.reparedestroy', $repare->id) }}"
                                                        method="POST" class="inline" id="delete-form-{{ $repare->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <div class="cursor-pointer mr-4"
                                                            onclick="confirmDelete({{ $repare->id }})">
                                                            <i
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="14" class="text-center py-4">Aucun matériel à réparer trouvé
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
