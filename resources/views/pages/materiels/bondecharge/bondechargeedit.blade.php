    ...
    <main class="h-full overflow-y-auto max-w-full pt-4">
        <div class="p-5">
            <div class="grid grid-cols-1 lg:gap-x-6 gap-x-0 lg:gap-y-0 gap-y-6">
                ...
                <form action="{{ route('materiels.bondechargeupdate', $bondecharge->id) }}" method="POST" class="mt-6">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="material_id" class="block text-sm font-medium text-gray-700">Numéro
                                d'inventaire:</label>
                            <input type="text" name="material" id="material" value="{{ old('material_id', $bondecharge->material_id ? $bondecharge->materiel->num_inventaire : 'N/A'  ) }}"
                            readonly
                            class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <x-input-error :messages="$errors->get('material_id')" class="mt-2" />
                        </div>
                        <input type="text" name="material_id" id="material_id" value="{{ old('material_id', $bondecharge->material_id) }}" class="hidden">

                        <div id="material-info">
                            <div>
                                <label for="qte" class="block text-sm font-medium text-gray-700">Quantité:</label>
                                <input type="number" name="qte" id="qte" value="{{ old('qte', $bondecharge->qte) }}"
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            ...
                            <div>
                                <label for="num_serie" class="block text-sm font-medium text-gray-700">Numéro de
                                    série:</label>
                                <input type="text" name="num_serie" id="num_serie" value="{{ old('num_serie', $bondecharge->num_serie) }}"
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            ...
                            <div>
                                ...
                                <input type="text" name="cedant" id="cedant" value="{{ old('cedant', $bondecharge->cedant_id ? $bondecharge->cedant->nom : '') }}"
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                <x-input-error :messages="$errors->get('cedant')" class="mt-2" />
                            </div>
                            <!-- Champ caché pour envoyer l'ID du service -->
                            <input type="hidden" name="cedant_id" id="cedant_id" value="{{ old('cedant_id', $bondecharge->cedant_id) }}">
                            <div>
