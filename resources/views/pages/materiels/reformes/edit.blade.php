...
@section('title', 'Modifier un matériel réformé')
@section('csslink')
    ...
    <main class="h-full overflow-y-auto max-w-full pt-4">
        <div class="p-5">
            <div class="grid grid-cols-1 lg:gap-x-6 gap-x-0 lg:gap-y-0 gap-y-6">
                ...
                <form action="{{ route('reforme.reformeupdate', $reforme->id) }}" method="POST" class="mt-6">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="material" id="material"
                                value="{{ old('material_id', $reforme->material_id ? $reforme->materiel->num_inventaire : 'N/A') }}"
                                readonly
                        ...
                        <input type="text" name="material_id" id="material_id"
                            value="{{ old('material_id', $reforme->material_id) }}" class="hidden">

                        <div id="material-info">
                            <div>
                                <label for="qte" class="block text-sm font-medium text-gray-700">Quantité:</label>
                                <input type="number" name="qte" id="qte" value="{{ old('qte', $reforme->qte) }}"
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                ...
                                <select name="cedant_id" id="cedant_id"
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}"
                                            {{ old('cedant_id', $reforme->cedant_id) == $service->id ? 'selected' : '' }}>
                                            {{ $service->nom }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="cessionnaire_id" id="cessionnaire_id"
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}"
                                            {{ old('cessionnaire_id', $reforme->cessionnaire_id) == $service->id ? 'selected' : '' }}>
                                            {{ $service->nom }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea name="designation" id="designation" rows="3"
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('designation', $reforme->designation) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="motif" id="motif" rows="3"
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('motif', $reforme->motif) }}</textarea>
